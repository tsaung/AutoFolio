...
3. As the AI, analyze the structural changes and autonomously write a commit message following the **Conventional Commits** format:
   ...
   > **Important:** Before running any git commit commands, detect the host operating system. Depending on the shell (e.g., PowerShell on Windows vs. Bash on macOS/Linux), ensure the commit message string is formatted correctly to avoid syntax errors (e.g., handling multiple `-m` flags or multiline strings appropriately for the shell).
